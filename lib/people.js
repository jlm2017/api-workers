...
const updatePersonInAPI = co.wrap(function*(nbPerson) {
  ...
  try {
    person = yield api.get_resource({resource: 'people', id: nbPerson.id, authInfo});
  } catch (err) {
    person = null;
  ...
  if (person === null) {
    ...
    try {
      yield api.post_resource('people', props, {authInfo});
    } catch (err) {
    ...
    if (utils.anyPropChanged(person, props)) {
      ...
      try {
        yield api.patch_resource('people', person, props, authInfo);
      } catch (err) {
